...
import { type InputParameters, VerboseError } from './classes.js'
...
describe('header', () => {
  test('should return the correct header with patch name', () => {
    expect(infos.header('test')).toContain('🎉 test 🎉')
  })

  test('should return the correct header with wild card', () => {
    expect(infos.header()).toContain('🎉 Success 🎉')
  })
})

describe('listNextSteps', () => {
  test('should add all infos (except ikarus)', () => {
  ...
  test('should add only basic infos including ikarus', () => {
    const patch: InputParameters = {
      name: 'Test-patch',
      needsVersions: [1],
      needsScripts: false,
  ...
  test('should add only basic infos without ikarus and lego', () => {
    const patch: InputParameters = {
      name: 'Test-patch',
      needsVersions: [1, 2],
      needsScripts: false,
...
describe('suggestTopics', () => {
  ...
  test('should return fewer topics', async () => {
    const patch = {
      needsVersions: [1],
      needsNinja: false,
      needsScripts: false,
